...
namespace UnityEditor.U2D.Sprites
{
    internal abstract partial class SpriteFrameModuleBase : SpriteEditorModuleModeSupportBase
    {
        ...
        protected Toggle m_NameToggleLock;
        protected Toggle m_PositionToggleLock;
        protected Toggle m_BorderToggleLock;
        protected Toggle m_PivotToggleLock;
        event Action<EEditCapability, bool> m_OnEditCapabilityChanged;
        ...
        private void AddMainUI(VisualElement mainView)
        {
            ...
            m_NameToggleLock = SetupToggleLock("nameToggleLock", m_SelectedFrameInspector.Q("spriteName"), EEditCapability.EditSpriteName);
            m_PositionToggleLock = SetupToggleLock("positionToggleLock", m_SelectedFrameInspector.Q("positionField"), EEditCapability.EditSpriteRect);
            m_BorderToggleLock = SetupToggleLock("borderToggleLock", m_SelectedFrameInspector.Q("borderField"), EEditCapability.EditBorder);
            m_PivotToggleLock = SetupToggleLock("pivotToggleLock", m_SelectedFrameInspector.Q("pivotData"), EEditCapability.EditPivot);
        ...
        Toggle SetupToggleLock(string elementName, VisualElement field, EEditCapability editCapability)
        {
            var toggle = m_SelectedFrameInspector.Q<Toggle>(elementName);
            toggle.RegisterValueChangedCallback(x =>
            {
                m_OnEditCapabilityChanged?.Invoke(editCapability, !toggle.value);
                field.SetEnabled(!toggle.value);
            });
            return toggle;
        }

        public void RegisterEditingCapabilityChangedCallback(Action<EEditCapability, bool> callback)
        {
            m_OnEditCapabilityChanged += callback;
        }

        public void UnregisterEditingCapabilityChangedCallback(Action<EEditCapability, bool> callback)
        {
            m_OnEditCapabilityChanged -= callback;
        }

        public void SetEditingCapabilityUI(EditCapability editCapability)
        {
            m_NameToggleLock.value = !editCapability.HasCapability(EEditCapability.EditSpriteName);
            m_PositionToggleLock.value = !editCapability.HasCapability(EEditCapability.EditSpriteRect);
            m_BorderToggleLock.value = !editCapability.HasCapability(EEditCapability.EditBorder);
            m_PivotToggleLock.value = !editCapability.HasCapability(EEditCapability.EditPivot);
            PopulateSpriteFrameInspectorField();
        }
        ...
        protected void PopulateSpriteFrameInspectorField()
        {
            ...
            m_NameToggleLock.SetEnabled(containsMultipleSprites);
            m_NameField.SetValueWithoutNotify(selectedSpriteName);
            m_PositionElement.SetEnabled(containsMultipleSprites);
            m_PositionToggleLock.SetEnabled(containsMultipleSprites);

            if (!containsMultipleSprites)
            {
                m_NameToggleLock.SetValueWithoutNotify(true);
                m_PositionToggleLock.SetValueWithoutNotify(true);
            }
        ...
        protected void HandlePivotHandle()
        {
            ...
            bool canEdit = !m_PivotToggleLock.value;
        ...
        protected void HandleBorderSidePointScalingSliders()
        {
            ...
            bool canEdit = !m_BorderToggleLock.value;
